...
import "@/styles/pixel-snow.css";

type SnowUniforms = {
  uTime: { value: number };
  uResolution: { value: Vector2 };
  uFlakeSize: { value: number };
  uMinFlakeSize: { value: number };
  uPixelResolution: { value: number };
  uSpeed: { value: number };
...
function getSnowUniforms(material: ShaderMaterial): SnowUniforms {
  return material.uniforms as SnowUniforms;
}
...
export function PixelSnow({
  ...
  const handleResize = useCallback(() => {
    ...
    resizeTimeoutRef.current = setTimeout(() => {
      ...
      (getSnowUniforms(material).uResolution.value).set(w, h);
    }, 100);
  }, []);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (isVisibleRef.current && materialRef.current) {
        getSnowUniforms(materialRef.current).uTime.value = (performance.now() - startTime) * 0.001;
  ...
  useEffect(() => {
    ...
    const uniforms = getSnowUniforms(material);
    uniforms.uFlakeSize.value = flakeSize;
    uniforms.uMinFlakeSize.value = minFlakeSize;
    uniforms.uPixelResolution.value = pixelResolution;
    uniforms.uSpeed.value = speed;
    uniforms.uDepthFade.value = depthFade;
    uniforms.uFarPlane.value = farPlane;
    uniforms.uBrightness.value = brightness;
    uniforms.uGamma.value = gamma;
    uniforms.uDensity.value = density;
    uniforms.uVariant.value = variantValue;
    uniforms.uDirection.value = (direction * Math.PI) / 180;
